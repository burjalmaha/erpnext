...
- **How it works:**
  ...
  5. Items with no stock balance to check — non-stock items (goods sold by weight that are not stock-tracked, for example) and Product Bundles — are passed over and can always be added.
  6. The balance used is the warehouse quantity itself. Sales made at the registers earlier in the day but not yet consolidated at day end are *not* deducted from it, so the register keeps selling against the warehouse figure. Completing the sale is stricter and does deduct them, so a quantity close to the full warehouse balance can still be refused at that point.
